...
public class AuthorizationServerConfig {
    ...
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http
        ...
        http.requestMatcher(endpointsMatcher).authorizeHttpRequests((authorize) -> ((AuthorizeHttpRequestsConfigurer.AuthorizedUrl) authorize.anyRequest()).authenticated())
//                .csrf((csrf) -> {
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtCustomizer(IUserDetailsHandle userDetailsHandle) {
        return context -> {
            ...
            if (context.getTokenType().equals(OAuth2TokenType.ACCESS_TOKEN)) {
                ...
                // 授权码模式的用户
                User codeUser;
                if (context.getPrincipal() instanceof CustomizeAuthenticatedToken) {
                    CustomizeAuthenticatedToken customizeAuthenticationToken = (CustomizeAuthenticatedToken) context.getPrincipal();
                    ...
                    UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = (UsernamePasswordAuthenticationToken) context.getPrincipal();
                ...
                if (userDetails != null) {
                    if (userDetails instanceof OAuth2UserDetails) {
                        OAuth2UserDetails oAuth2UserDetails = (OAuth2UserDetails) userDetails;
